...
## US COVID Cases Data
US-state data is in csv format and we can read it directly into R. It contains cumulative COVID case numbers by states of each day. We want to see the daily new cases by subtract the cumulative data by the data of previous day for each state. With the new time series data of the daily changes in cases per state, we will use this information to make observations on how the pandemic is affecting each state and how the country is handling coronavirus as a whole. 
