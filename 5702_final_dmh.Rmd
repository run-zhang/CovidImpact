...
```{r}
state_case = read.csv("./data/us-states.csv")
...
ggplot() +
  ...
  ggtitle("Year-over-year Comparison of seated dinner in the US")
```
### merge covid cases data and restaurant data
```{r}
df_case_res <- merge(x = US_case_by_date, y = US.nation.res, by.x = "date", by.y = "Date")
```

### plot 2 lines together
### example
```{r}
ggplot(df_case_res, aes(x = date)) +
  geom_line(aes(y=daily_new), color="red") +
  geom_hline(yintercept=0, color="orange", size=.5)+ 
  geom_line(aes(y=Percentage*2000),color="#69b3a2") +
  scale_y_continuous(name="daily new cases", sec.axis=sec_axis(~./2000, name="res percentage")) +
  theme(
